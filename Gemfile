...
group :development do
  ...
  if Gem::Requirement.create(['< 3.0.0']).satisfied_by?(Gem::Version.new(RUBY_VERSION.dup))
    gem 'facterdb', '~> 2.1',
        require: false
  end
  if Gem::Requirement.create(['>= 3.0.0']).satisfied_by?(Gem::Version.new(RUBY_VERSION.dup))
  ...
  gem 'racc', '~> 1.4.0',                        require: false if Gem::Requirement.create(['>= 2.7.0',
                                                                                            '< 3.0.0']).satisfied_by?(Gem::Version.new(RUBY_VERSION.dup))
...
group :system_tests do
  gem 'CFPropertyList', '< 3.0.7', require: false, platforms: %i[mswin mingw x64_mingw]
